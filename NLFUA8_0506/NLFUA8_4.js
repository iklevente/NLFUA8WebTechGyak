$(document).ready(function () {
  $.getJSON("NLFUA8_orarend.json", function (data) {
    var kurzusok = [];
    for (var i = 0; i < data["IKL orarend"].kurzus.length; i++) {
      var kurzus = data["IKL orarend"].kurzus[i];
  ...
  $("#megjelenit").click(function () {
    ...
    $.getJSON("NLFUA8_orarend.json", function (data) {
      ...
      for (var i = 0; i < data["phone number"].length; i++) {
        $("#orarend").append(
          "<li>" +
            data["phone number"][i].type +
            ": " +
            data["phone number"][i].number +
            "</li>"
        );
      }
      $("#orarend").append("</ul></p>");
      ...
      let elsoKurzus = data["IKL orarend"].kurzus[0].kurzusnev;
      $("#orarend").append("<p><b>Kurzusnév:</b> " + elsoKurzus + "</p>");

      let elsoOktato = data["IKL orarend"].kurzus[0].oktato;
      $("#orarend").append("<p><b>Okatató: </b>" + elsoOktato + "</p>");

      let szak = data["IKL orarend"].kurzus[0].szak;
      $("#orarend").append("<p><b>Szak: </b>" + szak + "</p>");

      elsoKurzus = data["IKL orarend"].kurzus[0];
      ...
      let helyszin = data["IKL orarend"].kurzus[0].helyszin;
